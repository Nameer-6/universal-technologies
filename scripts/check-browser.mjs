...
try {
  for (const [width, height] of [[1440, 1000], [390, 844], [320, 740]]) {
    ...
    await delay(1300)
    const metrics = await evaluate(`({ width: innerWidth, scrollWidth: document.documentElement.scrollWidth, h1: document.querySelector('h1')?.innerText, primary: document.querySelector('.btn-ink')?.getBoundingClientRect().toJSON(), cards: document.querySelectorAll('#services .service-card').length })`)
  ...
  await delay(700)
  assert.equal(await evaluate(`document.querySelectorAll('script[type="application/ld+json"]').length`), 2) // Organization + WebSite
  ...
  await evaluate(`document.querySelector('.nav-toggle').click()`)
  // The starting theme follows the machine's colour-scheme preference, so assert the toggle flips it.
  const darkBefore = await evaluate(`document.documentElement.hasAttribute('data-theme')`)
  await evaluate(`document.querySelector('.theme-toggle').click()`)
  assert.equal(await evaluate(`document.documentElement.hasAttribute('data-theme')`), !darkBefore)
  assert.equal(await evaluate(`document.querySelector('.theme-toggle').hasAttribute('aria-pressed')`), false)
